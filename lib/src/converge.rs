...
async fn converge_trees(
    ...
    let mut value_recorder = ValueRecorder::new(async |commit_id: &CommitId| {
...
{
    let mut value_recorder = ValueRecorder::new(async |commit_id: &CommitId| {
        value_fn(graph.get_commit(commit_id)?).await
    });
    let dominator_value =
        find_dominator_value(graph, divergent_commits, &mut value_recorder).await?;
    let dominator_producer = get_value_producer(repo, graph, &dominator_value, &value_recorder)?;

    let mut merge_builder = MergeBuilder::default();
    // ADD
    merge_builder.extend([(*dominator_value).clone()]);
    for divergent_commit in divergent_commits {
        let commit_value = value_recorder.get_value(divergent_commit.id()).unwrap();
        // REMOVE, ADD
...
async fn find_dominator_value<T, VF>(
    graph: &TruncatedEvolutionGraph,
    divergent_commits: &[Commit],
    value_recorder: &mut ValueRecorder<CommitId, T, VF>,
    ...
    VF: AsyncFn(&CommitId) -> Result<T, ConvergeError>,
{
    ...
    let dominator_value = graph
        .flow_graph
        .find_dominator_value_with_recorder(divergent_commit_ids.as_slice(), value_recorder)
        .await
...
/// produces a given description). The value must be present in value_recorder.
fn get_value_producer<T, VF>(
    repo: &Arc<ReadonlyRepo>,
    truncated_evolution_graph: &TruncatedEvolutionGraph,
    value: &Rc<T>,
    value_recorder: &ValueRecorder<CommitId, T, VF>,
) -> Result<CommitId, IndexError>
where
    T: Eq + Hash,
    VF: AsyncFn(&CommitId) -> Result<T, ConvergeError>,
{
    let producers = value_recorder.get_nodes_for_value(value).unwrap();
    match producers.len() {
        0 => unreachable!(), // If it is present in ValueRecorder, it comes from some commit.
